...
class StringFieldsTest {
  ...
  testFieldWithDefault_serialization() {
    assertValueIsSet(TestProto.newBuilder().setOptionalString('two').build());

    const proto3builder = TestProto3.newBuilder();
    assertValueIsCleared(proto3builder.setProto3String('').build());
    assertValueIsSet(proto3builder.setProto3String('two').build());
  }
